...
export function computeSensitivityRanking(
  ...
  if (hasMortgageA || hasMortgageB) {
    ...
    factors.push({
      ...
      impactType: "annual",
      impactLabelA: hasMortgageA ? `${annualDiff > 0 ? "-" : "+"}${formatAbs(annualDiff)}/yr on surplus` : "No mortgage held",
      impactLabelB: hasMortgageB ? `${annualDiff > 0 ? "-" : "+"}${formatAbs(annualDiff)}/yr on surplus` : "No mortgage held",
  ...
  factors.push({
    ...
    impactType: "annual",
    impactLabelA: `${formatAbs(impactExpA)}/yr on surplus`,
    impactLabelB: `${formatAbs(impactExpB)}/yr on surplus`,
  ...
  factors.push({
    ...
    impactType: "one-off",
    impactLabelA: `+£${shiftAmount.toLocaleString()} starting capital`,
    impactLabelB: `-£${shiftAmount.toLocaleString()} starting capital`,
